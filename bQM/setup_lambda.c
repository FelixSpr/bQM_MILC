...
void setup_lambda() {
  int i, j, k, l, count;
  complex inv_sqrt = cmplx(1.0, 0.0);
  complex i_inv_sqrt = cmplx(0.0, 1.0);
      ...
      for (k = 0; k < NCOL; k++) {
        for (l = 0; l < NCOL; l++) {
          if (k == i && l == j) {
            ...
            CSUM(Lambda[count].e[k][l], inv_sqrt);
            CDIF(Lambda[count + 1].e[k][l], i_inv_sqrt);
          }
          else if (k == j && l == i) {
            //CSUM(Lambda[count].e[k][l], i_inv_sqrt);
            //CDIF(Lambda[count + 1].e[k][l], inv_sqrt);
            CSUM(Lambda[count].e[k][l], inv_sqrt);
            CSUM(Lambda[count + 1].e[k][l], i_inv_sqrt);
          }
        }
      }
      count += 2;
  ...
  for (i = 0; i < NCOL - 1; i++) {
    ...
    inv_sqrt = cmplx(sqrt(2.0) / sqrt(k * (k + 1.0)),0.0);
    for (l = 0; l <= k; l++)
      Lambda[j].e[l][l] = inv_sqrt;
